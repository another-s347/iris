...
class Greeter(GreeterBase):
    ...
    async def Init(self, stream):
        ...
        for m in request.modules:
            try:
                ...
                print(e)
        await stream.send_message(NodeObjectRef(
            type="init",
            id=id(self.modules),
    ...
    async def GetParameter(self, stream):
        ...
        result = NodeObjectRef(location=self.args.name,type=str(type(parameters)))
    ...
    async def Call(self, stream):
        ...
        recursive = request.arg.recursive if request.arg else False
        args = tuple(map(self.map_args_to_local(recursive), args))
        kwargs = pickle.loads(request.arg.kwargs)
...
async def main(args):
    ...
    rpc.init_rpc(f"{args.name}", rank=args.rank, world_size=2)
